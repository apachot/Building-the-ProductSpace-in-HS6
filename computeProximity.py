...
for year in range(1995, 2021):
	data = pd.read_csv('output/M_cp_'+str(year)+'.csv', header=0, index_col=0)
	...
	Proximity_055 = [['source', 'target', 'weight']]
		...
		for j in range (i, len(hs_product_code)):
			...
			if ((sum_c_Mcp1 > 0) and (sum_c_Mcp2 > 0) and (sum_c_Mcp1Mcp2 > 0)):
				...
				if (P>.55):
					...
					Proximity_055.append([hs1, hs2, P])
					print("year", year, "i", i)
	...
	pd.DataFrame(Proximity).to_csv('output/HS6_Proximities_'+str(year)+'.csv', header=False, index=False)
	pd.DataFrame(Proximity_055).to_csv('output/HS6_Proximities_0.55_'+str(year)+'.csv', header=False, index=False)
	print("Fin")
